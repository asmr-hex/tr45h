...
export const Analyser = props => {
  const { audioScheduler } = useAudioContext()
  const [buffer, setBuffer] = useState(null)
  const [bufferLength, setBufferLength] = useState(null)
  const ref = useRef(null)
  
  useEffect(() => {
    if (audioScheduler === null) return

    audioScheduler.audio.output.analysis.fftSize = 2048
    const bufferLength = audioScheduler.audio.output.analysis.frequencyBinCount
  ...
  useEffect(() => {
    ...
    const { audio: { output: { analysis } } } = audioScheduler
  ...
  if (audioScheduler === null || buffer === null || bufferLength === null) return <AnalyserRoot/>
